...
program:
	declaration_list statement_list function_list comp_list {
...
function:
	KW_FUN KW_MAIN '(' ')' ':' function_body KW_NUF ';'
		{ $$ = template("int main () {\n%s\n}", $6); }
	| KW_FUN TK_IDENT '(' function_argument_list ')' KW_ARROW_FUNCTION data_type ':' function_body KW_NUF ';'
		{ $$ = template("%s %s(%s) {\n%s\n}", $7, $2, $4, $9); }
	| KW_FUN TK_IDENT '(' function_argument_list ')' ':' function_body KW_NUF ';'
		{ $$ = template("void %s(%s) {\n%s\n}", $2, $4, $7); }
	;
...
assign_statement:
	...
	/* compact array over integers: a <- [expr for i:size] : type; */
	| TK_IDENT KW_COLONEQ '[' expr KW_FOR TK_IDENT ':' TK_NUMBER ']' ':' data_type ';'
		{ $$ = template("%s* %s = (%s*)malloc(%s * sizeof(%s));\nfor (int %s=0; %s < %s; ++%s){\n%s[%s] = %s;\n}",
		                $11, $1, $11, $8, $11, $6, $6, $8, $6, $1, $6, $4); }
	/* compact array over another array: a <- [expr for x:type in array of size] : new_type; */
	| TK_IDENT KW_COLONEQ '[' expr KW_FOR TK_IDENT ':' data_type KW_IN TK_IDENT KW_OF TK_NUMBER ']' ':' data_type ';'
		{
			char* res = (char *)malloc(strlen($10) + 16);
...
if_statement:
	KW_IF '(' expr ')' ':' if_for_while_body KW_FI ';'
		{ $$ = template("if (%s) {\n%s\n}",$3,$6); }
	| KW_IF '(' expr ')' ':' if_for_while_body KW_ELSE ':' if_for_while_body KW_FI ';'
		{ $$ = template("if (%s) {\n%s\n} else {\n%s\n}",$3,$6,$9); }
	;
...
while_statement:
	KW_WHILE '(' expr ')' ':' if_for_while_body KW_OD ';'
		{ $$ = template("while(%s) {\n%s\n} ",$3,$6); }
	;
...
comp:
	KW_RECORD TK_IDENT ':' comp_body KW_DROCER ';'
